import { notFound } from "next/navigation";
import Link from "next/link";
import { venues } from "@/data/venues";
import { InterestForm } from "@/components/venue/interest-form";
import { VenueMap } from "@/components/venue/venue-map";
import { SupportForm } from "@/components/venue/support-form";
import { VenuePhoto } from "@/components/venue/venue-photo";
import { isInterestRegion } from "@/data/regions";
import { canonicalActivity } from "@/data/search-options";
import { venueLocation } from "@/lib/venue-results";
import { formatPriceFrom, priceFrom } from "@/data/venue-pricing";

type VenuePageProps = { params: Promise<{ slug: string }>; searchParams: Promise<{ regionInterest?: string; activity?: string }> };

export default async function VenuePage({ params, searchParams }: VenuePageProps) {
  const { slug } = await params;
  const { regionInterest, activity } = await searchParams;
  const venue = venues.find((item) => item.slug === slug);
  if (!venue) notFound();
  const location = venueLocation(venue);
  const startingPrice = priceFrom(venue.slug);
  const defaultInterestRegion = isInterestRegion(regionInterest) ? regionInterest : venue.zone;
  const requestedActivity = activity ? canonicalActivity(activity) : undefined;
  const defaultEventType = requestedActivity && venue.eventTypes.includes(requestedActivity) ? requestedActivity : venue.category;
  return <main className="min-h-screen bg-[var(--secondary)] px-5 py-8 sm:py-16"><div className="mx-auto max-w-6xl"><Link className="text-sm font-semibold text-[var(--primary)]" href="/buscar">← Voltar para a busca</Link><div className="mt-8 grid gap-8 lg:grid-cols-[1fr_25rem]"><section><VenuePhoto alt={venue.imageAlt} src={venue.image} /><p className="mt-8 text-sm font-semibold uppercase tracking-[0.14em] text-[var(--primary)]">{venue.region} · São Paulo</p><h1 className="mt-3 font-display text-4xl leading-none sm:text-5xl">{venue.name}</h1><p className="mt-5 max-w-2xl text-lg text-[var(--muted)]">{venue.summary}</p><dl className="mt-8 grid gap-4 rounded-3xl bg-white p-6 sm:grid-cols-3"><div><dt className="text-sm text-[var(--muted)]">Capacidade</dt><dd className="mt-1 font-semibold">Até {venue.capacity} pessoas</dd></div><div><dt className="text-sm text-[var(--muted)]">Indicado para</dt><dd className="mt-1 font-semibold">{venue.eventTypes.join(" · ")}</dd></div><div><dt className="text-sm text-[var(--muted)]">Valor</dt><dd className="mt-1 font-semibold">{startingPrice ? <>A partir de {formatPriceFrom(startingPrice)}<span className="font-normal text-[var(--muted)]"> · diária</span></> : "Sob consulta"}</dd></div></dl><section className="mt-8 rounded-3xl bg-white p-5 sm:p-6"><h2 className="font-display text-3xl">Localização aproximada</h2><p className="mt-2 text-sm text-[var(--muted)]">{venue.region}, São Paulo — a confirmação de endereço e disponibilidade acontece no atendimento.</p><VenueMap latitude={venue.latitude} longitude={venue.longitude} venueName={venue.name} /></section><SupportForm venueSlug={venue.slug} /></section><aside className="self-start"><InterestForm defaultEventType={defaultEventType} defaultGuests={venue.capacity > 150 ? 150 : 80} defaultInterestRegion={defaultInterestRegion} defaultLocation={location} venueSlug={venue.slug} /></aside></div></div></main>;
}
