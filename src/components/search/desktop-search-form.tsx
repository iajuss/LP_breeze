  ...
  return (
    <form className="hidden w-full grid-cols-[1.15fr_1fr_0.9fr_0.8fr_auto] rounded-2xl bg-white p-2 text-[var(--foreground)] shadow-xl lg:grid" onSubmit={onSubmit}>
      <div className="relative border-r border-[var(--border)] px-4 py-2 text-sm font-medium" ref={activityMenuRef}>
        ...
        {activityMenuOpen ? <div className="absolute top-full z-30 mt-2 w-[calc(100%-2rem)] rounded-xl bg-[var(--primary)] p-2 text-white shadow-2xl shadow-black/20" role="listbox">
      ...
      <label className="border-r border-[var(--border)] px-4 py-2 text-sm font-medium">Onde?
        <input aria-invalid={Boolean(errors.location)} className="mt-1 block w-full bg-transparent text-base font-semibold outline-none focus-visible:outline-[var(--primary)]" onChange={(event) => onChange({ ...values, location: event.target.value })} placeholder="Cidade, bairro ou região" value={values.location} />
      </label>
      <label className="border-r border-[var(--border)] px-4 py-2 text-sm font-medium">Quando?
        <input className="mt-1 block w-full bg-transparent text-base font-semibold outline-none focus-visible:outline-[var(--primary)]" onChange={(event) => onChange({ ...values, date: event.target.value })} type="date" value={values.date} />
      </label>
      <label className="px-4 py-2 text-sm font-medium">Pessoas
        <input aria-invalid={Boolean(errors.guests)} className="mt-1 block w-full [appearance:textfield] bg-transparent text-base font-semibold outline-none focus-visible:outline-[var(--primary)] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none" min="1" onChange={(event) => onChange({ ...values, guests: Number(event.target.value) })} placeholder="Convidados" type="number" value={values.guests || ""} />
      </label>
