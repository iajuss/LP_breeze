...
export function VenueSearch({ entryPoint }: VenueSearchProps) {
  ...
  const submit = () => {
    ...
    track("search_submitted", {
      entryPoint,
      source: entryPoint,
      ...(values.activity ? { eventType: values.activity } : {}),
      ...(canonicalLocation(values.location) ? { neighborhood: canonicalLocation(values.location)! } : {}),
      ...(values.guests ? { guestCount: values.guests } : {}),
