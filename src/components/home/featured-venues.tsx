    ...
    <section className="w-full scroll-mt-28 py-16 sm:py-20 lg:flex lg:min-h-screen lg:items-center" id="espacos">
      <div className="mx-auto w-full max-w-7xl px-5 lg:px-8">
        <div className="mb-10 flex items-end justify-between gap-4">
          ...
          <a className="font-semibold text-[var(--primary)]" href="/buscar">Ver todos os espaços →</a>
        </div>
        <div className="flex w-full min-w-0 snap-x snap-mandatory gap-5 overflow-x-auto pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:grid lg:grid-cols-3 lg:overflow-visible">
          {venues.slice(0, 6).map((venue, index) => <div className="w-[82vw] max-w-[320px] shrink-0 snap-start sm:w-[340px] sm:max-w-none lg:w-auto lg:max-w-none lg:min-w-0" key={venue.id}><VenueCard featured={index === 0} venue={venue} /></div>)}
        </div>
      </div>
    </section>
  );
